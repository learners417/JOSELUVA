...
// --- EL MOTOR DE DIAS ---
// Ritmo fijo: cada 7 dias reales desde el ingreso = 1 semana del programa.
// Dia 1-7 -> semana 1, dia 8-14 -> semana 2, etc. Tope en 12.
export function diasDesdeIngreso(state) {
  if (!state.createdAt) return 0;
  const inicio = new Date(state.createdAt);
  const hoy = new Date();
  const ms = hoy - inicio;
  return Math.max(0, Math.floor(ms / (1000 * 60 * 60 * 24)));
}

// La semana que el CALENDARIO habilita hoy (por fecha, ritmo fijo).
export function semanaPorFecha(state) {
  const dias = diasDesdeIngreso(state);
  const semana = Math.floor(dias / 7) + 1;
  return Math.min(semana, SEMANAS.length);
}

// El dia dentro de la semana actual del programa (1 a 7).
export function diaDeLaSemana(state) {
  return (diasDesdeIngreso(state) % 7) + 1;
}
...
// (Los checks de clases/practicas guian al usuario pero no bloquean:
// lo que cierra la semana es bajar la bitacora.)
...
// --- La semana ACTUAL ---
// La primera no completa, PERO nunca mas alla de lo que el calendario
// habilita por fecha (ritmo fijo de 7 dias). Asi, aunque complete todo
// rapido, el programa lo lleva semana a semana en su tiempo real.
export function semanaActual(state) {
  let primeraIncompleta = SEMANAS.length;
  for (const w of SEMANAS) {
    if (!semanaCompleta(w.n, state)) { primeraIncompleta = w.n; break; }
  }
  const tope = semanaPorFecha(state);
  return Math.min(primeraIncompleta, tope);
}
...
// --- ¿Termino todo lo de esta semana y espera a la proxima por fecha? ---
// True cuando completo la semana actual pero el calendario aun no abre la siguiente.
export function esperandoProximaSemana(state) {
  const n = semanaActual(state);
  return semanaCompleta(n, state) && semanaPorFecha(state) <= n && n < SEMANAS.length;
}
