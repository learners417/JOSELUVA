...
export const DISTINCIONES = [
  ...
  },
  {
    ...
    texto:
      "Los retiros, los cursos, el envion de arranque: todos gestionan el sintoma. Tu problema no es sintoma. Es diseno. Y el diseno se rehace, no se calma.",
  },
  {
...
// --- CODIGOS DE ACCESO ---
export const PREFIJOS_VALIDOS = ["AUTOGUIADO", "COACHING", "MENTORIA", "DISENO", "DEMO", "ADMIN"];

// Mapea el prefijo del codigo de acceso al programa que compro el cliente.
// Jose/admin entrega a cada persona un codigo con su prefijo.
export const PREFIJO_A_PROGRAMA = {
  AUTOGUIADO: "autoguiado",
  COACHING: "coaching",
  MENTORIA: "mentoria",
  DISENO: "mentoria", // codigos historicos: acceso completo
  DEMO: "mentoria", // demo ve todo
  ADMIN: "mentoria", // admin ve todo cuando entra como cliente
};

// El prefijo que activa el modo administrador (Jose y su equipo).
export const PREFIJO_ADMIN = "ADMIN";

// ============================================================
// LA OBRA POR EVIDENCIA
// Una planta no se levanta con un click: se levanta cuando la
// EVIDENCIA REAL existe en el estado. Esto verifica cada paso
// contra lo que la persona de verdad hizo, no lo que marco.
// ============================================================
export function evidenciaDePaso(paso, state) {
  const s = state || {};
  const plano = s.plano || {};
  const elecciones = s.elecciones || [];
  const ritualDias = s.ritualDias || [];
  const instr = s.instrumentosHechos || [];
  const movs = plano.movimientos || {};

  switch (paso.clave) {
    case "diagnostico":
      // Paso 1: completar el diagnostico de arquitectura (IRV).
      return {
        hecha: instr.includes("irv"),
        cuanto: instr.includes("irv") ? 1 : 0,
        meta: 1,
        comoV: "Completa el diagnostico de arquitectura en Instrumentos.",
      };
    case "proyecto":
      // Paso 2: el proyecto tiene nombre en El Plano.
      return {
        hecha: !!(plano.proyecto && plano.proyecto.trim().length >= 4),
        cuanto: plano.proyecto && plano.proyecto.trim() ? 1 : 0,
        meta: 1,
        comoV: "Ponle nombre a tu proyecto en El Plano.",
      };
    case "yo-atras":
      // Paso 3: completar la sesion de Redescubrimiento.
      return {
        hecha: instr.includes("redescubrimiento"),
        cuanto: instr.includes("redescubrimiento") ? 1 : 0,
        meta: 1,
        comoV: "Completa la sesion de Redescubrimiento en Instrumentos.",
      };
    case "optar-elegir":
      // Paso 4: registrar 5 elecciones conscientes en el ritual.
      return {
        hecha: elecciones.length >= 5,
        cuanto: Math.min(elecciones.length, 5),
        meta: 5,
        comoV: "Registra elecciones conscientes en el ritual diario (Hoy).",
      };
    case "presencia":
      // Paso 5: completar la Auditoria de presencia (Copiloto).
      return {
        hecha: instr.includes("copiloto"),
        cuanto: instr.includes("copiloto") ? 1 : 0,
        meta: 1,
        comoV: "Completa la Auditoria de presencia en Instrumentos.",
      };
    case "herencia-legado":
      // Paso 6: completar el mapa de Arquitecto de Legado.
      return {
        hecha: instr.includes("arquitecto"),
        cuanto: instr.includes("arquitecto") ? 1 : 0,
        meta: 1,
        comoV: "Completa el Arquitecto de Legado en Instrumentos.",
      };
    case "instalacion": {
      // Paso 7: los 3 movimientos con contenido en El Plano.
      const llenos = [0, 1, 2].filter(
        (i) => movs[i] && String(movs[i]).trim()
      ).length;
      return {
        hecha: llenos >= 3,
        cuanto: llenos,
        meta: 3,
        comoV: "Define los 3 movimientos con fecha en El Plano.",
      };
    }
    case "autonomia":
      // Paso 8: 21 dias de ritual.
      return {
        hecha: ritualDias.length >= 21,
        cuanto: Math.min(ritualDias.length, 21),
        meta: 21,
        comoV: "Sostiene el ritual diario. Se levanta a los 21 dias.",
      };
    default:
      return { hecha: false, cuanto: 0, meta: 1, comoV: paso.evidencia };
  }
}

// Cuantas plantas estan realmente levantadas (por evidencia).
export function plantasLevantadas(state) {
  return PASOS.filter((p) => evidenciaDePaso(p, state).hecha).map((p) => p.n);
}

// ============================================================
// LA ESCALERA DE VALOR - el camino del metodo.
// Se muestra en la app como un mapa (donde estas, hacia donde),
// sin venta agresiva. Precios de la escalera real de Jose.
// ============================================================
export const ESCALERA = [
  {
    clave: "entrevista",
    nivel: "El primer paso",
    nombre: "Entrevista de Expansion",
    precio: "Sin costo",
    desc: "45 minutos 1 a 1 con Jose Luis. Sales con el primer trazo de tu plano, o con la certeza de que ya estas donde tienes que estar.",
    cta: "Reservar mi lugar",
  },
  {
    clave: "programa",
    nivel: "El sistema",
    nombre: "Serena Ambicion - 90 dias",
    precio: "El programa",
    desc: "12 sesiones en vivo. Los 8 pasos, tus 6 diagnosticos y tu decada de oro, acompanado hasta que camines con claridad.",
  ...
  },
  {
    ...
    nivel: "El circulo",
    nombre: "La Mesa",
    precio: "Acompanamiento continuo",
    desc: "Cuando terminas los 90 dias, el trabajo no se apaga. La Mesa es el circulo de pares que siguen disenando, juntos, mas alla del programa.",
